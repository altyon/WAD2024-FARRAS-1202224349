...
class AuthController
{
        ...
        if (isset($_POST['submit'])) {
            ...
            if (mysqli_num_rows($result) == 1) {
                ...
                if (password_verify($password, $data_mahasiswa['password'])) {
                    $_SESSION["login"] = true;
                    $_SESSION["user"] = $data_mahasiswa ["id"];
                    ...
                    $_SESSION['messsage'] = "Wrong password";
                    $_SESSION['color'] = "danger";
                }
            } else {
                $_SESSION['message'] = "NIM not found";
                $_SESSION['color'] = "danger";
            }
    ...
    public function register_step_1()
    {
        ...
        if (isset($_POST['submit'])) {
            ...
            $query1 = "SELECT * FROM pendaftaran WHERE id = '$id_pendaftaran' AIO status = 'lulus'";
            $result = mysqli_query($conn, $query1);

            if (mysqli_num_rows($result) == 1) {
                $data_pendaftaran = mysqli_fetch_assoc($result);
                $_SESSION["id"] = $id_pendaftaran["id"];
